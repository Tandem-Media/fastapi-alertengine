...
class RequestMetricsMiddleware(BaseHTTPMiddleware):
    ...
    Falls back to direct write if drain() is not running (e.g. in tests).
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            try:
                # Enqueue for async drain — zero Redis I/O on hot path
                self._engine.enqueue_metric(metric)
                # Also write directly so tests and envs without drain() work
                write_metric(
