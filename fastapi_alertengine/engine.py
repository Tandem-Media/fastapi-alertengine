...
class AlertEngine:
    ...
    def __init__(self, redis=None, config=None):
        ...
        # ── v1.6: Action suggestions ──────────────────────────────────────────
        # Rolling buffer of recent suggestions — for status endpoint display
        self._recent_suggestions: collections.deque = collections.deque(maxlen=20)
        # Closed-loop pipeline stage tracker (informational only)
        self._pipeline_stage: str = "detect"   # detect→evaluate→suggest→authorize→log
    ...
    def get_action_suggestions(
        ...
        ev = self.evaluate()
        hs = ev.get("health_score", {})
        score = hs.get("score", 100.0) if isinstance(hs, dict) else 100.0

        self._pipeline_stage = "suggest"
    ...
    def start(self, app, *, health_path: str = "/health/alerts"):
        ...
        def _status():
            ...
            return {
                ...
                "pipeline":             engine.get_pipeline_status(),
            }
