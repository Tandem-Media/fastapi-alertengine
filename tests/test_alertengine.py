...
def config():
    return AlertConfig(
        stream_key="test:metrics", stream_maxlen=1_000,
        p95_warning_ms=500.0, p95_critical_ms=1_000.0,
        error_rate_warning_pct=2.0, error_rate_critical_pct=5.0, error_rate_baseline_pct=0.5,
    )
...
class TestAlertEventSchema:
    def test_no_data_returns_ok_with_empty_alerts(self, engine):
        r = engine.evaluate()
        assert r["status"] == "ok" and r["reason"] == "no_data"
        assert r["alerts"] == [] and r["system_health"] == 100.0
    ...
    def test_metrics_field_names_match_advertised_json(self, engine, rdb, config):
        ...
        d = engine.evaluate()
        assert "p95_latency_ms" in d["metrics"] and "p50_latency_ms" in d["metrics"]
        assert "error_rate_percent" in d["metrics"] and "request_count_1m" in d["metrics"]
    def test_timestamp_is_iso8601_utc_string(self, engine, rdb, config):
        write_metric(rdb, config, "/api/x", "GET", 200, 50.0)
        r = engine.evaluate()
        assert isinstance(r["timestamp"], str) and "T" in r["timestamp"] and r["timestamp"].endswith("Z")
    ...
    def test_system_health_is_float_0_to_100(self, engine, rdb, config):
        write_metric(rdb, config, "/api/x", "GET", 200, 50.0)
        r = engine.evaluate()
        assert isinstance(r["system_health"], float) and 0.0 <= r["system_health"] <= 100.0
...
class TestAlertClassification:
    def test_ok_under_all_thresholds(self, engine, rdb, config):
        for _ in range(50): write_metric(rdb, config, "/api/fast", "GET", 200, 50.0)
        r = engine.evaluate()
